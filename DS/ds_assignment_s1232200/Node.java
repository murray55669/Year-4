...
public class Node extends Thread {
		...
		*/ 
		...
		if (chunks[0].equals("ELECTION")) {
			...
			if (this.getNodeId() == incId) {
				//If we are the leader, the election has completed - log the event
				this.network.log += "LEADER "+this.getNodeId()+"\n";
			} else {
	...
	public void tick(int round) {
		...
		for (Integer election : this.elections) {
			if (round == election) {
				if (this.participant) {
					System.out.println("Node "+this.getNodeId()+" starting election; already a participant!");
